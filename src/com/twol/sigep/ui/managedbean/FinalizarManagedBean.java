...
public class FinalizarManagedBean {
	...
	public FinalizarManagedBean() throws IOException {
		try {
			f = new Facede();
			int id_venda = SessionUtil.getVendaAFinalizar();
			aFinalizar = f.buscarVendaPeloId(id_venda);
			aFinalizar.getDia();
	...
	public void finalizarVendaAVista() {
		if (this.pago >= aFinalizar.getTotal()) {
			...
			try {
				...
				if (ret) {
					SessionUtil.putNextMensagem(new FacesMessage(
							...
							"Venda finalizada à vista com sucesso",
							"Venda Finalizada"));
				} else {
					SessionUtil.putNextMensagem(new FacesMessage(
			...
			FacesContext.getCurrentInstance().addMessage(
					null,
					(new FacesMessage(FacesMessage.SEVERITY_ERROR,
							"Valor inferior ao total da venda",
							"Valor Invalido")));
		}
	}

	
	private boolean ja_chamou = false;
	public synchronized void finalizarVendaAPrazo() throws IOException {
		if (ja_chamou) {
			// SessionUtil.redirecionarParaPage("venda.jsf");
			if (SessionUtil.getNextMensagem() != null) {
				SessionUtil.putNextMensagem(SessionUtil.getNextMensagem());
				SessionUtil.redirecionarParaPage("venda.jsf");
			}
		...
		if (this.partePaga >= aFinalizar.getTotal()) {
			FacesContext.getCurrentInstance().addMessage(
					null,
					(new FacesMessage(FacesMessage.SEVERITY_ERROR,
							"Valor iqual ou superior ao total da venda",
							"Valor Invalido")));
			return;
		}
		try {
			...
			FacesContext.getCurrentInstance().addMessage(
					null,
					(new FacesMessage(FacesMessage.SEVERITY_ERROR,
							"Selecione o cliente antes de finalizar a venda",
							"Cliente Invalido")));
			return;
		}
		...
		try {
			if(!observacao.isEmpty()){
				aFinalizar.setObservacao(observacao);
			}
			...
			if (ret) {
				SessionUtil
						.putNextMensagem(new FacesMessage(
								FacesMessage.SEVERITY_INFO,
								"Venda finalizada!\nCliente "
										+ selecionado.getNome()
										+ " \nDébito: "
										+ OperacaoStringUtil
												.formatarStringValorMoedaComDescricao(selecionado
														.getDebito()),
								"Venda Finalizada"));
			} else {
				SessionUtil
			...
			SessionUtil.redirecionarParaPage("venda.jsf");
			SessionUtil.removerVendaAFinalizar();
		} catch (IOException e) {
			e.printStackTrace();
			...
			try {
				f.recalcularDebitoDoCliente(selecionado);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public List<ItemDeVenda> getItens() {
		List<ItemDeVenda> itens = f.buscarItensDaVendaPorIdDaVenda(this.aFinalizar.getId());
	...
	public void setPago(String pago) {
		try {
			if (pago.length() >= 3) {
				pago = pago.replace(".", "").replace(" ", "").replace(",", "");
				pago = pago.substring(0, pago.length() - 2) + ","
						+ pago.substring(pago.length() - 2);
			}
			this.pago = Double.valueOf(pago.replace(".", "").replace(" ", "")
					.replace(",", "."));
		} catch (NumberFormatException ne) {
			SessionUtil.putNextMensagem(new FacesMessage(
					FacesMessage.SEVERITY_ERROR, "Formato de valor errado",
					"Formato errado"));
			this.pago = 0;
		}
	}
		...
		try {
			if (partePaga.length() >= 3) {
				partePaga = partePaga.replace(".", "").replace(" ", "")
						.replace(",", "");
				partePaga = partePaga.substring(0, partePaga.length() - 2)
						+ "," + partePaga.substring(partePaga.length() - 2);
			}
			this.partePaga = Double.valueOf(partePaga.replace(".", "")
					.replace(" ", "").replace(",", "."));
		} catch (NumberFormatException ne) {
			this.partePaga = 0;
		}
